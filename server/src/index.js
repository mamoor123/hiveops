...
io.on("connection", (socket) => {
	...
	socket.on("disconnect", () =>
		log.info({ user: socket.user.name }, "💤 User disconnected"),
	);
});
...
async function seedDefaults() {
	const deptCount = (
		await db.prepare("SELECT COUNT(*) as count FROM departments").get()
	).count;
	if (deptCount === 0) {
		const defaults = [
			{
			...
			{
				name: "Marketing",
				description: "Marketing, branding, and growth",
				icon: "📈",
				color: "#ec4899",
			},
			{
			...
			{
				name: "Design",
				description: "Creative and visual design",
				icon: "🎨",
				color: "#ef4444",
			},
		];
		for (const d of defaults)
			await db
				...
				.run(d.name, d.description, d.icon, d.color);
