...
router.post("/test-llm", authMiddleware, async (req, res) => {
	...
	if (!testKey)
		return res.json({
			success: false,
			message: "No API key provided. Using simulated responses.",
		});
	try {
				...
				messages: [
					{
						role: "system",
						content:
							'You are a helpful assistant. Reply with just "Connection successful!"',
					},
		...
		if (!response.ok) {
			...
			return res.json({
				success: false,
				message: `API error: ${response.status} — ${err.slice(0, 200)}`,
			});
		}
